		...
		implements ISessionStore<TKey>, IKeyValueStore<TKey, TValue>
{
	 ...
	 * Set to CONSTANT.TIMEOUT_INFINITE to disable expiration.
	 *
	...
	public KeyValueSessionStore(int expirationMillis, int pollingIntervalMillis)
	{
		...
		if(expirationMillis >= 0)
			expirationTimer.scheduleAtFixedRate(new ExpirationCheckingTimerTask(), new LocalDateTime().plusMillis(expirationMillis).toDateTime().toDate(), pollingIntervalMillis);
	}
	 ...
	 */
	public KeyValueSessionStore(int expirationMillis, int pollingIntervalMillis, Class<?> genericTypeParameterKey, Class<?> genericTypeParameterValue)
	{
		setExpirationMillis(expirationMillis);
		setPollingIntervalMillis(pollingIntervalMillis);
	 ...
	 * i.e. removes the item from the collection.
	 * This is an O(log2(n)) operation.
	 *
	 * @throws NullPointerException Key is null
	 */
	...
	/**
	 * Returns all keys.
	 * Result is in ascending Key order.
	 * This is an O(1) operation.
	 */
	@Override
	...
	/**
	 * Returns all values.
	 * Result is in ascending Key (not value) order.
	 * This is an O(1) operation.
	 */
	@Override
	 ...
	 * If the value is CONSTANT.TIMEOUT_INFINITE then there is no expiration.
	 */
	@Override
	 ...
	 * If the value is CONSTANT.TIMEOUT_INFINITE then there is no polling.
	 */
	@Override
	 ...
	 * Returns true if the key was found and its value was replaced.
	 * This is an O(log2(n)) operation.
	 *
	 * @throws NullPointerException Key is null
	 */
	...
	/**
	 * Sets the duration of a session in milliseconds.
	 * If the value is CONSTANT.TIMEOUT_INFINITE then there is no expiration.
	 *
	...
	/**
	 * Scans all objects and removes the ones that have expired.
	 * This is an O(nlog2(n)) operation.
	 */
