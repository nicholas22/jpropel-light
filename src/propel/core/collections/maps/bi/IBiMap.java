...
/**
 * A type-aware bi-directional AVL-tree-backed map. BiMaps require that keys and values are unique and can be accessed by key or by value.
 * 
    ...
    extends ReifiedMap<TKey, TValue>
{
  ...
  /**
   * Adds a key/value pair in the AvlBiMap, returning true if succeeded. Null keys/values are not allowed.
   * 
  ...
  /**
   * Returns the value associated with a key, if existent. Otherwise returns null.
   */
  TValue getByKey(TKey key);

  /**
   * Returns the key associated with a value, if existent. Otherwise returns null.
   */
