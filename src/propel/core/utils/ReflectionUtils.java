...
public final class ReflectionUtils
{
   ...
   /**
    * Compares two Method classes and returns true if they
    * appear to be referring to the same signature of a method.
    *
    * @throws NullPointerException An argument is null
    */
   ...
   /**
    * Compares two Field classes and returns true if they
    * appear to be referring to the same signature of a field.
    *
    * @throws NullPointerException An argument is null
    */
   ...
   /**
    * Compares two Constructors and returns true if they
    * appear to be referring to the same constructor.
    *
    * @throws NullPointerException An argument is null
    */
   ...
   /**
    * Compares two class members and returns true if they
    * appear to be referring to the same member.
    *
    * @throws NullPointerException An argument is null
    */
   ...
   /**
    * Compares two Method classes and returns true if they
    * appear to be referring to the same signature of a property.
    *
    * @throws NullPointerException An argument is null
    */
    ...
    * This allows to determine the calling class, method name, etc.
    */
    ...
    * TODO: fix bug where having a whitespace in the contained folder causes %20 to be used (when this used as part of a JAR)
    *
    * @throws NullPointerException	 An argument is null.
           ...
           throws ClassNotFoundException
   {
      ...
      if (StringUtils.contains(fullPath, '/'))
         normalizedFullPath = StringUtils.replace(StringUtils.replace(fullPath, "%5c", "/", StringComparison.OrdinalIgnoreCase), "%2f", "/", StringComparison.OrdinalIgnoreCase);
      else
         if (StringUtils.contains(fullPath, '\\'))
            normalizedFullPath = StringUtils.replace(StringUtils.replace(fullPath, "%5c", "\\", StringComparison.OrdinalIgnoreCase), "%2f", "\\", StringComparison.OrdinalIgnoreCase);
      ...
      if (directory.exists()) {
         ...
         try {
            ...
            while (entries.hasMoreElements()) {
               ...
               if (entryName.endsWith(classExt) && entryName.startsWith(relPath) && entryName.length() > (relPath.length() + CONSTANT.FORWARD_SLASH.length())) {
                  String className = entryName.replace(CONSTANT.FORWARD_SLASH_CHAR, CONSTANT.DOT_CHAR).replace(CONSTANT.BACK_SLASH_CHAR, CONSTANT.DOT_CHAR).replace(classExt, CONSTANT.EMPTY_STRING);
    ...
    * @throws NullPointerException An argument is null.
    */
   public static <T extends Annotation> List<T> getMethodAnnotations(Method annotatedMethod, Class<T> annotationType, boolean includeInherited)
   {
      if (annotatedMethod == null)
         throw new NullPointerException("annotatedMethod");
   ...
   /**
    * Returns all interfaces of a type, except the specified as a generic argument one (T).
    * Note that this does not return inherited interfaces.
    *
    * @throws NullPointerException An argument is null.
    */
    ...
    * You may specify whether to include inherited properties in the search of the property name.
    *
   ...
   public static void injectProperty(Object obj, String propertyName, Object propertyValue, boolean includeInherited)
   {
      ...
      if (pi == null)
         throw new IllegalArgumentException("The object of type " + obj.getClass().getName() + " does not have a property named '" + propertyName + "'");

      // obtain setter and inject value
      try {
         pi.getSetter().invoke(obj, propertyValue);
      } catch (Throwable e) {
         throw new IllegalArgumentException("Could not inject property value on object of type " + obj.getClass().getName() + " and property " + propertyName, e);
      }
   }

   /**
    * Returns true if any of the type's BaseTypes are equal to the given type or if the two types are the same.
    * The method scans up until the root of the object hierarchy.
    * Only classes are checked (no interfaces).
    *
    ...
    * The method does not check for inheritance relationships. 
    * Rather it checks if duck typing would be possible 
    * (i.e. if it looks like a duck and quacks like a duck, then it is a duck).
    * This is useful for checking if an annotation "implements" some interface,
    * because in Java annotations cannot extend or implement other types. 
    * Note: even if this returns true, it does not mean that you can perform casting. 
    * You may specify whether all or only declared methods are to be checked.
    */
   public static boolean isLookingLike(final Class<?> type, Class<?> interfaceType, final boolean includeTypeInherited, boolean includeInterfaceTypeInherited)
   {
      if (type == null)
         throw new NullPointerException("type");
    ...
    * all the way up. No classes are checked (only interfaces).
    *
    * @throws NullPointerException When an argument is null
    */
    ...
    * This cannot be used for arrays, see the ReifiedArray class for that.
    */
   ...
   /**
    * Allows dynamic dispatch on any object using a specified interface.
    * I.e. method calls to interface methods are routed to the given run-time object.
  ...
  public static <T> T proxy(final Object obj, Class<T> interfaceType)
   ...
   /**
    * Internal-use function, matches a method in the declared methods of a class,
    * returning the method found in the class instead of the method given. 
